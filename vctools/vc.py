...
class VCTools:
    ...
    VERSIONS = {'2012': 11, '2013': 12, '2015': 14, '15': 15}
    COMMANDS = [
        'check_call',
        'devenv',
        'msbuild',
    ...
    def __init__(self, version_name=None, platform='x86'):
        ...
        Args:
            version_name (str or None): Visual Studio version name ('2012',
                '2013', '2015', etc.). If None, it uses the most recent
    ...
    def get_target_platform(self):
        ...
        return self.environ.get('PLATFORM', 'X86')
    ...
    def __get_vsvars(self, toolset_version, platform):
        try:
            vs_tools_env_name = 'VS{}0COMNTOOLS'.format(toolset_version)
            vs_tools_path = os.environ.get(vs_tools_env_name)
            if vs_tools_path is None:
                raise ValueError('The environment variable {} is not found'
                                 .format(vs_tools_env_name))
            vcvarsall_path = os.path.normpath(os.path.join(vs_tools_path,
                                                           '..',
                                                           '..',
                                                           'VC',
                                                           'vcvarsall.bat'))
            if not os.path.exists(vcvarsall_path):
                raise ValueError('Invalid path "{}" to script vcvarsall.bat'
                                 .format(vcvarsall_path))
        except ValueError as e:
            raise self.NotInstalledError('Visual Studio {} is not installed: {}'
                                         .format(toolset_version, e.args))
        ...
        if popen.returncode != 0 or stdout or stderr:
            raise self.PlatformError('\n'.join([stdout.decode('cp866'),
                                                stderr.decode('cp866')]))
